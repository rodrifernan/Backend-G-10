	...
	body("userName")
		...
		.custom((value, { req }) => {
			return User.findOne({ where: { userName: value } }).then((user) => {
				if (user) {
					if (req.route.path === "/update" && req.user.id === user.id)
						return;
...
router.post("/", userValidators, async (req, res, next) => {
	...
	try {
		...
		return res
			.status(201)
			.send({ success: true, msg: "Registro Completo" });
	} catch (error) {
		next(error);
	}
});
...
router.post(
	...
	[
		loginVerification,
		...userValidators.filter(
			(item, index) => ![1, 2, 7, 9].includes(index)
		),
	],
	async (req, res, next) => {
		const errors = validationResult(req);
...
router.put("/", loginVerification, async (req, res) => {
	const { firstName, lastName, email, address, phone, userName, id } =
		req.body;
